#!/usr/bin/env bash
# Create an orcest orchestrator VM on Proxmox using a Ubuntu cloud image.
#
# Usage:
#   create-orchestrator-vm.sh <vm-id> <vm-name> [storage]
#
# Examples:
#   create-orchestrator-vm.sh 199 orcest-orchestrator
#   create-orchestrator-vm.sh 199 orcest-orchestrator local-zfs
#
# Environment overrides:
#   ORCEST_VM_USER=thayne    ORCEST_VM_MEMORY=4096
#   ORCEST_VM_BRIDGE=vmbr0   ORCEST_VM_CORES=2
#   ORCEST_VM_SOCKETS=1      ORCEST_VM_DISK=20G
#
# Prerequisites:
...
MEMORY="${ORCEST_VM_MEMORY:-4096}"
CORES="${ORCEST_VM_CORES:-2}"
SOCKETS="${ORCEST_VM_SOCKETS:-1}"
DISK_SIZE="${ORCEST_VM_DISK:-20G}"
...
# --- Summary ---
echo "=== Creating orcest orchestrator VM ==="
echo "  VMID:    $VMID"
echo "  Name:    $VMNAME"
...
if [[ -n "${IP:-}" ]]; then
    ...
    echo "  orcest provision-orchestrator ${IP} --user ${CI_USER}"
else

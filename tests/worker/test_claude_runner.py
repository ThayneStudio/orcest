...
def test_extract_summary_last_result_in_multi_result_stream_regression_111():
    ...
    Regression test for issue #111 (originally fixed in PR #59):
    _extract_summary must scan all lines and return the *last* result,
    not early-return on the first one.
    """
    lines = [
...
def test_is_usage_exhausted_rate_limit_not_at_eol():
    ...
    passed to it.  This test confirms that empty stderr returns False and
    documents the stderr-only contract.
    """
    # empty string -> _RATE_LIMIT_RE doesn't match; no false positive
